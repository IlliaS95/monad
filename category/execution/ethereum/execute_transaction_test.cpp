...
TYPED_TEST(TraitsTest, irrevocable_gas_and_refund_new_contract)
{
    ...
    static auto const initial_balance = 56'000'000'000'000'000;
    static auto const actual_gas_cost = [] {
    ...
    static auto const gas_limit = actual_gas_cost + 2'000;
    static auto const max_fee_per_gas = 10;
    ...
    {
        State state{bs, Incarnation{0, 0}};
        auto final_balance = intx::be::load<uint256_t>(state.get_balance(from));
        if constexpr (TestFixture::is_monad_trait()) {
            if constexpr (TestFixture::Trait::monad_rev() == MONAD_ZERO) {
                ...
                EXPECT_EQ(
                    final_balance,
                    uint256_t{initial_balance - gas_limit * max_fee_per_gas});
            }
        }
        else {
            EXPECT_EQ(
                final_balance,
                uint256_t{initial_balance - actual_gas_cost * max_fee_per_gas});
        }
...
TYPED_TEST(TraitsTest, refunds_delete)
{
    ...
    static constexpr auto gas_charged_tx1 = [] {
        auto const pre_istanbul_cost = 41'092;
        // Gas decreased due to calldata cost reduction in Istanbul
        // where gas per non-zero byte was reduced from 68 to 16
        auto const istanbul_cost = 41'040;
        // Gas increased due to storage repricing in Berlin
        auto const berlin_cost = 43'140;
    ...
    // 0 -> 0 -> Z
    {
...

        {
            ...
            auto const final_balance = final_balance_uint256[0];
...
TYPED_TEST(TraitsTest, refunds_delete_then_set)
{
    ...
    // X -> X -> 0 then X -> 0 -> X
    {
        static constexpr auto gas_limit_tx =
            50'000; // actual gas used + low gas SSTORE stipend
