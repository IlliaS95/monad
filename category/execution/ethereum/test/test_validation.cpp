...
TYPED_TEST(TraitsTest, optional_fields_existence)
{
    static constexpr auto base_fee_per_gas =
        value_since<typename TestFixture::Trait, uint256_t>(EVMC_LONDON);
    static constexpr auto withdrawals_root =
        value_since<typename TestFixture::Trait, bytes32_t>(EVMC_SHANGHAI);
    static constexpr auto blob_gas_used =
        value_since<typename TestFixture::Trait, uint64_t>(EVMC_CANCUN);
    static constexpr auto excess_blob_gas =
        value_since<typename TestFixture::Trait, uint64_t>(EVMC_CANCUN);
    static constexpr auto parent_beacon_block_root =
        value_since<typename TestFixture::Trait, bytes32_t>(EVMC_CANCUN);
    static constexpr auto requests_hash =
        value_since<typename TestFixture::Trait, bytes32_t>(EVMC_PRAGUE);
